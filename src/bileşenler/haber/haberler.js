...
const data = [
  ...
  {
    baslik: "2023 haziran ayında Mezun olabilecekmiyiz",
    tarih: "1 Ekim 2022",
    ilkParagraf: `Evet arkadaşlar merhaba 2 ay sonra eğitime başlayacağız bu yüzden biraz heyecan var ama kursun nasıl geçeceğine dair pek bir fikrim yok 
          umarım sınıf arkadaşlarımla iyi anlaşabilir ve verilen ödevlerde yardımlaşabiliriz konuları iyice anlarsakta tadından yenmez`,
...
function haberyapici(haber) {
  const articleEkle = document.createElement("div");
  articleEkle.setAttribute("class", "article");

  const h2Ekle = document.createElement("h2");
  h2Ekle.textContent = haber.baslik;
  articleEkle.appendChild(h2Ekle);

  const tarihEkle = document.createElement("p");
  tarihEkle.textContent = haber.tarih;
  articleEkle.appendChild(tarihEkle);
  tarihEkle.setAttribute("class", "article date");

  const paragraf1ekle = document.createElement("p");
  paragraf1ekle.textContent = haber.ilkParagraf;
  articleEkle.appendChild(paragraf1ekle);

  const paragraf2ekle = document.createElement("p");
  paragraf2ekle.textContent = haber.ikinciParagraf;
  articleEkle.appendChild(paragraf2ekle);

  const paragraf3ekle = document.createElement("p");
  paragraf3ekle.textContent = haber.ucuncuParagraf;
  articleEkle.appendChild(paragraf3ekle);

  const spanEkle = document.createElement("span");
  spanEkle.setAttribute("class", "article expandButton");
  spanEkle.textContent = "+";
  articleEkle.appendChild(spanEkle);

  document.querySelector("expandButton");
  spanEkle.addEventListener("click", () => {
    articleEkle.classList.toggle("article-open");
  });
  return articleEkle;
}
const haber3 = document.querySelector(".articles");
data.forEach((element) => {
  haber3.appendChild(haberyapici(element));
});
